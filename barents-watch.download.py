...
if "BARENTS_WATCH_ACCESS_TOKEN" not in os.environ:
...
def acquire_token():
    if "BARENTS_WATCH_CLIENT_ID" not in os.environ:
        raise ValueError("Missing BARENTS_WATCH_CLIENT_ID in env")
    if "BARENTS_WATCH_CLIENT_SECRET" not in os.environ:
    ...
    response = requests.post(
        "https://id.barentswatch.no/connect/token", headers=headers, data=data
    )
        ...
        for idx, line in enumerate(response.iter_lines()):
            if line:
                ...
                try:
                    ...
                    iso_format = "".join(
                        m.groups()[0] + "." + milliseconds + m.groups()[2]
                    )
